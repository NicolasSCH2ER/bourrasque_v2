...
try:
    sc = make_scene()
    sph = setup_sphere_emitter(turbulence=0.0, seed=1)
    ...
    for f in range(15):
        ...
        if positions.shape[0] > 0:
            ...
            occupants = positions
        site_idx = ops._site_indices(sites, spacing)
        if occupants.shape[0] > 0:
            occ_idx = ops._site_indices(occupants, spacing)
            free_mask = ~np.isin(ops._void_rows(site_idx), ops._void_rows(occ_idx))
        else:
            free_mask = np.ones(sites.shape[0], dtype=bool)
        n_occ = occupants.shape[0]
        n_free = int(free_mask.sum())
        n_occupied_sites = n_sites - n_free
        deficit = max(0, n_sites - n_occ)
        print(f"f={f}: n_occ(particles in bbox)={n_occ}  n_occupied_sites={n_occupied_sites}  n_free={n_free}  deficit={deficit}")

        emitted = harness._emit_inflow_sites()
        sim.step(frame_dt)
    sim.destroy()
finally:
    unregister()
