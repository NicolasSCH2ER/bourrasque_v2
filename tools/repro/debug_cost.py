...
try:
    ...
    # warmup (frame 0, generates first slabs)
    ...
    print(f"emit_inflow_sites: mean={times.mean():.3f}ms  max={times.max():.3f}ms  p95={np.percentile(times,95):.3f}ms")
    sim.destroy()
finally:
    unregister()
